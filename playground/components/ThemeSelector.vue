...
<template>
  <div>
    ...
    <div class="grid grid-cols-8 gap-4">
      <div v-for="color in colors" :key="color" class="flex cursor-pointer text-gray-12 items-center space-x-2" @click="() => canvas = color">
    ...
    <div>Primary</div>
    <div class="grid grid-cols-8 gap-4">
      <div v-for="color in colors" :key="color" class="flex cursor-pointer text-gray-12 items-center space-x-2" @click="() => primary = color">
        <div class="size-4 rounded-full" :class="`bg-${color}-9`" />
        <div>{{ color }}</div>
      </div>
    </div>
